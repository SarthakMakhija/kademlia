...
impl Node {
    ...
    pub(crate) fn new_with_id(endpoint: Endpoint, id: NodeId) -> Self {
        Node { id, endpoint }
    }
...
mod tests {
    ...
    fn node_equals_itself() {
        let node = Node::new(Endpoint::new("localhost".to_string(), 2330));
        assert!(node.eq(&node))
    }
    ...
    fn node_equals_other_node() {
        let node = Node::new(Endpoint::new("localhost".to_string(), 2330));
        let other_node = Node::new(Endpoint::new("localhost".to_string(), 2330));
    ...
    fn node_does_not_equal_other_node() {
        let node = Node::new(Endpoint::new("localhost".to_string(), 2330));
        let other_node = Node::new(Endpoint::new("localhost".to_string(), 1982));
